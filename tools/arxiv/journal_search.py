...
OUTPUT_TXT = os.path.join(OUTPUT_DIR, "journal_results.txt")
OUTPUT_MD = os.path.join(OUTPUT_DIR, "JOURNAL.md")
...
def fetch_europepmc(query: str, max_results: int = MAX_RESULTS) -> list[dict]:
    """Fetch results from Europe PMC REST API."""
    results = []
    cursor = "*"
...
def write_md(papers: list[dict], path: str, topic: str, days: int) -> None:
    ...
    lines.extend([
        "",
        "Auto-generated by `python tools/arxiv/journal_search.py`.",
        "",
...
def main() -> None:
    ...
    for topic in topics:
        ...
        if args.all_topics:
            txt_path = os.path.join(OUTPUT_DIR, f"journal_results_{topic}.txt")
            md_path = os.path.join(OUTPUT_DIR, f"JOURNAL_{topic}.md")
        else:
            txt_path = OUTPUT_TXT
            md_path = OUTPUT_MD
    ...
    if args.all_topics and len(topic_results) > 1:
        ...
        combined_txt = os.path.join(OUTPUT_DIR, "journal_results_all.txt")
        combined_md = os.path.join(OUTPUT_DIR, "JOURNAL_all.md")
